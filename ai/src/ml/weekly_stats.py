...
logger = logging.getLogger("SonLokma.WeeklyStats")
...
# ── Groq Client Kontrolü ──────────────────────────────────────────────────────
if not GROQ_API_KEY:
    logger.warning("⚠️ GROQ_API_KEY bulunamadı! weekly_stats LLM olmadan çalışacak.")
    client = None
else:
...
def groq_cagir(sistem_prompt, kullanici_mesaji, sicaklik=0.5):
    if client is None:
        return "Bu hafta iyi bir performans gösterdin! Tasarrufların devam ediyor. 🎉"
    try:
        ...
        logger.error(f"❌ Groq hatası (weekly_stats): {e}")
        return "Bu hafta iyi bir performans gösterdin! Tasarrufların devam ediyor. 🎉"
...
def run():
    ...
    try:
        response = requests.get("http://127.0.0.1:8000/analytics/order-history", timeout=10)
